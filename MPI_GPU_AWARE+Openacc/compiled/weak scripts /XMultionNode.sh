...
do
    ...
    for j in {1..7}
    do
        mpirun -np $i  ../topoGPU_X.exe < ../"input${i}" >> $output
    done
...
do
    ...
    for j in {1..7}
    do
        mpirun  -np $i  ../topoMULTICORE_X.exe < ../"input${i}" >> $output
    done
...
do
    ...
    for j in {1..7}
    do
        mpirun --map-by socket --bind-to core -np $i  ../topoHOST_X.exe < ../"input${i}" >> $output
    done
