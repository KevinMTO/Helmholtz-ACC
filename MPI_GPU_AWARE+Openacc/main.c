...
void Init(struct JacobiData *data, int *argc, char **argv)
{
    ...
    MPI_Init_thread(argc, &argv, MPI_THREAD_MULTIPLE, &provided); //MPI+ OPENMP
    //#########################################################################
    //#########################################################################
...
/*CREATE 2D TOPOLOGY*/
    ...
    //printf("MPI: %d HERE IS NEW %ld\n",data->iMyRank,data->Topo_comm);

    int coords[2];
    
    MPI_Comm_rank(data->Topo_comm,&(data->iMyRank));
